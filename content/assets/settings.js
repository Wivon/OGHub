    ...
    settingsContent = [
        {
            ...
            "paramContent": '<div class="panelContainer containerANA stepOne"><h2>Let\'s add an App !</h2><h5>Select the app you want to add</h5><div class="mainContainer"><div class="fakeCard"><img onclick="MainActionANA()" src="img/logoX512.png"></img><p>your-app.exe</p></div><div class="editor"><input class="newCardNameInput" onkeyup="document.querySelector(\'.containerANA p\').textContent = this.value" type="text"></input><input style="font-size: 16px;" class="newCardArgsInput" type="text" placeholder="arguments"></input></div></div><div class="buttons"><button onclick="backANA()" class="btn-small hidden back"><</button><button class="selectExeBtn ActionBtnANA" onclick="MainActionANA()">Select an app</button><button onclick="NextANA()" class="btn-small disabled next">></button></div></div>',
            "openWith": "settings"
        }, {
            "paramId": "customize",
            ...
            "paramId": "updates",
            "paramName": "Updates",
            "paramContent": `<div class="panelContainer container-only"><div class="updateHeader"><img src="img/settings_update_icon.png"><div><h2>✅ You\'re up to date</h2><h4>Last check: <span>${LastUpdateCheckDate}</span></h4><div class="buttons"><button class="checkBtn" onclick="CheckForUpdatesButton()">Check for Updates</button><button class="hidden restartBtn" onclick="restartApp()">restart</button><p class="downloadProgress"><span class="dlPercent"></span><progress class="dlProg" value="0" max="0" style="width:325px; height: 22px; margin-right: 12px; margin-left: 12px;"></progress><span class="dlSpeed"></span></p></div></div></div><hr style="width: 97%; margin-left: auto; margin-right: auto;"><div class="updateInfo"><h3>Last release changelog :</h3><p class="changelog">${lastReleaseDescription}</p></div><div class="buttonsFooter"><h3 onclick="shell.openExternal(\'https://github.com/Wivon/OGHub\')">Current version: ${appVersion}</h3><a onclick="shell.openExternal(\'https://wivon-hub.tk\')">&copy Wivon Hub</a><h3 onclick="shell.openExternal(\'https://forms.gle/jmFMQZCknfSwKmmN6\')">feedback</h3></div></div>`,
            "openWith": "settings",
